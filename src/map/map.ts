...
export class Map{
	...
	constructor(filename:string){
		...
		let meshData:MeshData = json.decode(love.filesystem.read(filename)[0])
		...
		this.mesh = love.graphics.newMesh(vertices,"triangles","static")
	}
	...
	pick(x:number,y:number){
		...
		if(this.closest==null){
			...
			this.setQuadColor(this.mesh,this.closest,Colors.Red)
			return
		}
		if(this.closest == result[1]){
			return
		}
		this.setQuadColor(this.mesh,this.closest,Colors.Black)
		this.closest =result[1]
		this.setQuadColor(this.mesh,this.closest,Colors.Red)
	}

	setQuadColor(mesh:Mesh,quad:number,c:Color){
